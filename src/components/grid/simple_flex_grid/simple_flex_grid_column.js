...
function SimpleFlexGridColumn(props: PropsTypes) {
    const {size, full, style} = props;

    const flexBasis: string = is(String, size) ? size : `${size}%`;
    const flexGrow: number = full === true ? 1 : 0;

    return <div className={props.classes.componentContainer} style={{flexBasis, flexGrow, ...style}}>
        {props.children}
    </div>
}
