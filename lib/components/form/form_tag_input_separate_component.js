...
function FormTagInputSeparateComponent(props) {
  ...
  var addTagFromTextInput = function addTagFromTextInput() {
    ...
    if (_immutable.List.isList(value)) {
      var valueNotExist = (0, _ramda.isNil)(value.find(function (listValue) {
        var data = listValue.get('data');
        return !(0, _ramda.isNil)(data) && data === textInputValue;
      }));
      ...
      var _valueNotExist = (0, _ramda.findIndex)((0, _ramda.propEq)('data', textInputValue))(value) === -1;
  ...
  var onClickTagCloseButton = function onClickTagCloseButton(event, currentTagData) {
    ...
    if (_immutable.List.isList(value)) {
      var tagIndex = value.findIndex(function (listValue) {
        var data = listValue.get('data');
        return !(0, _ramda.isNil)(data) && data === currentTagData;
      });
      onChange(value.delete(tagIndex));
    } else {
      var _tagIndex = (0, _ramda.findIndex)((0, _ramda.propEq)('data', currentTagData))(value);

      onChange((0, _ramda.remove)(_tagIndex, 1, value));
    }
  };
